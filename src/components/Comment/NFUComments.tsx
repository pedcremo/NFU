...
import { IonButton, IonToast, IonItem, IonInput } from "@ionic/react";
...
const NFUComments: React.FC<CommentProps> = (props) => {
  ...
  const event = Object.keys(state.events)
    .map((key) => state.events[key])
    .find((event) => event.id == props.gameID);
    ...
    <>
      <form method="post" onSubmit={addComment} className="form_add_comment">
        ...
        <IonButton
          expand="block"
          type="submit"
          className="form_add_comment--button"
        >
